...
url = "http://192.168.170.1:8090/rest/experimental/search?user=sec&cql=siteSearch+~+" + \
...
class Wiki(object):
    def __init__(self):
        ...
        # ref: https://stackoverflow.com/questions/8908287/why-do-i-need-b-to-encode-a-string-with-base64/41437531
    ...
    def get_page(self, num=10000):
        result = []
        proxies = {
                'http': 'http://192.168.170.1:8087','https': 'https://192.168.170.1:8087'
                }
        for i in range(0, num, 500):
            resp = requests.get(url.format(str(i)), headers=self.headers, proxies=proxies, verify=False)
            if not resp.content:
                print(resp.content)
